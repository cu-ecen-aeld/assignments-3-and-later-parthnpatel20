            ...
            size_t char_offset, size_t *entry_offset_byte_rtn )
{
    ...
    // Iterate over circular buffer entries
    for (size_t i = 0; i < num_entries; i++)
    {
        struct aesd_buffer_entry *entry = &buffer->entry[index];

        // If char_offset falls within this entry
        ...
        // Update offset and move to next index
        offset += entry->size;
        index = (index + 1) % AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED;
    }

    return NULL; // char_offset exceeds available data
}
...
void aesd_circular_buffer_add_entry(struct aesd_circular_buffer *buffer, const struct aesd_buffer_entry *add_entry)
{
    ...
    // Check if buffer is full; if so, move out_offs to the next entry
    if (buffer->full)
    {
        buffer->out_offs = (buffer->out_offs + 1) % AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED;
    }

    // Add new entry at in_offs position
    buffer->entry[buffer->in_offs] = *add_entry;

    // Move in_offs forward
    buffer->in_offs = (buffer->in_offs + 1) % AESDCHAR_MAX_WRITE_OPERATIONS_SUPPORTED;

    // If in_offs == out_offs after increment, buffer is full
